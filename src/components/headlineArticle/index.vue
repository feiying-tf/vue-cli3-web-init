<template>
  <div class="wrapper">
    <Title content="头条文章"></Title>
    <PageLoading v-if="pageLoading"></PageLoading>
    <div v-else>
      <router-link v-for="item in newsArr" :key="item.id" :to="{path: `/newsDetail/${item.id}`}" :target="isOpenInNew?'_blank':'_self'">
        <img v-if="item.imgPath" v-lazy="item.imgPath" alt="易酒">
...
import { HeadLinesAdapt } from '@/common/adaptation.ts';
  ...
  getHeadLines () {
    ...
    fetchNews(obj).then(res => {
      ...
      this.newsArr = res.data.rows.map((item: any) => {
        return new HeadLinesAdapt(item)
      })
    }).catch(() => {
      this.pageLoading = false;
    });
